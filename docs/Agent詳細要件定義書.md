...
### 10.5 プロンプトインジェクション対策

取得ページとユーザー発話の双方を、モデルへの入力としては信頼しない。

- モデルへ渡す外部文字列は、リクエストごとの乱数IDを持つデリミタで囲む。本文中の
  デリミタ文字は無効化し、入力側からブロックを閉じられないようにする
- システム命令は、タスク説明の前後の両方に防御文を置く（指示防衛・サンドイッチ）
- モデルへ渡す前に決定論的な検出を行う。ユーザー発話で検出した場合はモデルを呼ばずに
  定型文で応答し、Runを起動しない
- 取得ページで検出した場合は記録のみとし、イベントは破棄しない。破棄すると、ページに
  一行加えるだけで正当なイベントを隠せてしまう
- システム命令にカナリアトークンを埋め、応答に現れた場合はその応答を破棄する。
  カナリアの値はログへ出さない
- モデルが返した自由文（`interestsPrompt` 等）も検証してから採用する

実装と判断の根拠は [ADR-004](./ADR-004-プロンプトインジェクション対策.md) に記載する。
